...
describe("mission planning drafts", () => {
  ...
  it("serves the operator mission workspace javascript bundle", async () => {
    ...
    expect(response.text).toContain("Open source JSON");
      ...
      expect(response.text).toContain("workspaceFocusTarget");
    ...
    expect(response.text).toContain("Review regulatory matrix");
    expect(response.text).toContain("Drill-down links support accountable review and evidence navigation only.");
    expect(response.text).toContain("They do not certify compliance, submit records, or replace operator judgement.");
    expect(response.text).toContain('id="post-operation-evidence-pack"');
    expect(response.text).toContain('id="pre-signoff-evidence-summary"');
    expect(response.text).toContain('id="rendered-report-readiness-summary"');
    expect(response.text).toContain("#timeline-panel");
    expect(response.text).toContain("#regulatory-matrix-panel");
  ...
  it("serves the operator live operations javascript bundle", async () => {
    ...
    expect(response.text).toContain("renderMapViewStateEvidenceHistory");
      ...
      expect(response.text).toContain("getDrilldownFocusFromLocation");
      expect(response.text).toContain("mapEvidenceId");
      expect(response.text).toContain("conflictAcknowledgementId");
      expect(response.text).toContain("Focused source record");
      expect(response.text).toContain("This focus supports accountable audit review only");
